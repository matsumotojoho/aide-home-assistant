...
公開URLで運用しているため、ログインできること = 家の操作 (玄関の解錠承認を含む) ができることになる。
総当たりを実用的な速度で試せないよう、IP単位で制限する。

- 15分以内に5回失敗すると、そのIPを15分ロック (HTTP 429)
- 失敗ごとに500msの遅延
- ロック状態はメモリ保持 (再起動でリセットされるが、速度を落とす目的は達成できる)
- 成功すると失敗カウントは消える

`src/rateLimit.ts` / `test/rateLimit.test.ts`。プロキシ配下のため実クライアントは
`X-Forwarded-For` の先頭から取る。

## 既知の残課題

- セッション失効管理 (現状はCookie期限30日のみ。端末紛失時は SESSION_SECRET を変えれば全端末が失効する)
- Google OAuth等へのログイン方式の置換 (AuthServiceを差し替える形で対応可能)
- 二要素認証 (仕様書25で「Face ID等は必須にしない」方針のため未実装)
