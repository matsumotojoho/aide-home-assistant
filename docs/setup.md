...
連携後、**PWAの設定タブ→デバイス登録→「Home Assistantから取り込む」**を押すと、HAにある機器を走査して未登録のものを一覧表示する。チェックして登録すれば entity_id を手で打つ必要はない。

- 部屋は表示名から推定する (「寝室のカーテン」→ 寝室)。HAのREST APIはエリアを返さないため、外れたら登録後に編集する
- センサーは温度・湿度・ドア・人感など意味のあるものだけ候補に出し、既定ではチェックを外してある
- HA側と名前が違う機器、HAに見つからない機器も同じ画面に出る。名前の上書きは既定オフ (Aide側の名前を手で付け直していることが多いため)

entity_id を直接指定したい場合は、同じ画面の入力欄から従来どおり手で登録できる。日本語名と部屋はRouterの高速分類に使われる。
