...
> 前提: Railway等へデプロイ済みで `https://<ドメイン>/alexa` が公開されていること。
> AlexaはHTTPS必須のため localhost では動作しません。
...
4. **Endpoint** を設定
   - Endpoint → **HTTPS**
   - Default Region: `https://<あなたのRailwayドメイン>/alexa`
...
| 言い方 | 動作 |
|---|---|
| ありがとう / ありがとうございます | 「はい、またどうぞ。」と言って終了 |
| もういいよ / もう大丈夫 / 以上 | 同上 |
| ストップ / やめて / キャンセル | 同上 (Alexa標準のStopIntent/CancelIntent) |
| 終わり / おしまい | Alexaが `SessionEndedRequest` として処理し、無言で終了 |
| 何も言わない | 約8秒でセッションが切れる |

`SessionEndedRequest` には音声を返してはいけない仕様なので、サーバーは空応答を返す。

**注意**: 「ありがとう」は既定ではCatchAllIntentに吸われてClaude行きになり、
「スマホに通知しますね」と返ってしまう。AMAZON.StopIntentのサンプル発話に
明示的に登録すること (`ops/alexa/interaction-model.json`)。
サーバー側にも保険として `CLOSING_RE` の判定を置いている。
